...
dialog_subbasin::dialog_subbasin
	(wxWindow *pParent,
	SubBasin *basin,
	SWATControlRecord *control,
	swat_controller *controller)
	: wxDialog (pParent, -1, "Subbasin") // , wxDefaultPosition, wxSize (400, 600))
{
 	wxBoxSizer *stack = new wxBoxSizer( wxVERTICAL );

	wxPanel *panel = new wxPanel(this, wxID_ANY);

    notebook = new notebook_subbasin (panel, basin, control, controller, wxSize (600, 550));
...
BEGIN_EVENT_TABLE(dialog_subbasin, wxDialog)
END_EVENT_TABLE()

void dialog_subbasin::EndModal
	(int retCode)
